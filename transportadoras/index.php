...
<script> 
    $(document).ready(function() {	
		var
		  acao = '';
		  id   = '';		  
		 $('#dados').load('transportadoras/listar.php');		  
	     $( "#tabs" ).tabs(); 
		...
		//Inserir Alterar registro 
		  $('#salvar').click(function(){
		   ...
		   $.post('transportadoras/salvar.php',{pessoa: $("#pessoa:checked").val(), cnpj: $("#cnpj").val(), tipo_ie : $("#tipo_ie option:selected").val(), ie: $("#ie").val(), nome: $("#nome").val(), fantasia: $("#fantasia").val(), telefone: $("#telefone").val(), cep: $("#cep").val(), endereco: $("#endereco").val(), numero: $("#numero").val(), bairro: $("#bairro").val(), complemento: $("#complemento").val(), estado: $("#estado option:selected").text(), codigo_estado: $("#estado option:selected").val(), cidade: $("#cidade option:selected").text(), codigo_cidade: $("#cidade option:selected").val(), email: $("#email").val(), acao : acao, id: id}, function(resposta){
				alert(resposta);	
				$("#sucesso").dialog();		
                 $('#dados').load('transportadoras/listar.php');
				 $('#abandonar').click();
		 ...
		 $('.botaoAlterar').live('click', function () {
			...
			id = codigo;     
			  $.getJSON('transportadoras/carregardados.php?codigo='+codigo, function(registro){					  
...
$('.botaoExcluir').live('click', function () {	
 if( confirm('Deseja excluir?') ){
	...
	//$("#aguarde2").html("<img src='images/loader.gif' alt='Entrando...' />").fadeIn("slow");
		$.post('transportadoras/excluir.php', {codigo: codigo}, function(resposta) {
...
<h2><img src="imgs/transportadoras.png" width="24" height="24" />Cadastro de Transportadoras</h2>
  <div id="tabs">
    <ul> 
