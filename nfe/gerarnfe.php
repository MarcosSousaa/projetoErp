...
$nfe = new MakeNFe();
$nfeTools = new ToolsNFe('../nfephp/config/config'.$id_emitente.'.json');


//Buscar dados da Nota Fiscal Selecionada
include_once("../includes/conexao.php");

$numeroNota = $_POST["numero_nota"];
$notaselecionada = mysqli_query($conexao,"select * from nota1 where id = '$numeroNota' and id_emitente = '$id_emitente'");
$dadosNota = mysqli_fetch_array($notaselecionada);
$id_nota    = $dadosNota["id_nota"];
//Dados da NFe - infNFe
$cUF      = $_SESSION["emitente"]["codigo_estado"]; //35 - codigo numerico do estado do Emitente
$cNF      = str_pad($dadosNota["id"], 8, '0', STR_PAD_LEFT);//'00000010'; //numero aleatório da NF
$natOp    = $dadosNota["natureza"];//'Venda de Produto'; //natureza da operação
$indPag   = $dadosNota["formapagto"];; //0=Pagamento à vista; 1=Pagamento a prazo; 2=Outros
$mod      = '55'; //modelo da NFe 55 ou 65 essa última NFCe
$serie    = '1'; //serie da NFe
$nNF      = $dadosNota["id"];//'10'; // numero da NFe
$dhEmi    = date("Y-m-d\TH:i:sP",strtotime($dadosNota["emissao"])); //date("Y-m-d\TH:i:sP");//Formato: “AAAA-MM-DDThh:mm:ssTZD” (UTC - Universal Coordinated Time).
$dhSaiEnt = date("Y-m-d\TH:i:sP", strtotime($dadosNota["saida"]));//date("Y-m-d\TH:i:sP");//Não informar este campo para a NFC-e.
$tpNF     = '1';
$idDest   = '1'; //1=Operação interna; 2=Operação interestadual; 3=Operação com exterior.
$cMunFG   = $_SESSION["emitente"]["codigo_cidade"];//'3512001' Código do Municipio do emitente;
...
$tpAmb = $_SESSION["ambiente"];  //1=Produção; 2=Homologação
$finNFe = $dadosNota["finalidade"];; //1=NF-e normal; 2=NF-e complementar; 3=NF-e de ajuste; 4=Devolução/Retorno.
...
$verProc = '1.0'; //versão do aplicativo emissor
...
$xLgr     = $_SESSION["emitente"]["endereco"];//'Av. Rio de Janeiro';
$nro      = $_SESSION["emitente"]["numero"];//'s/n';
$xCpl     = $_SESSION["emitente"]["complemento"];//'Qd. 38 Lt. 4,5 e 34';
$xBairro  = $_SESSION["emitente"]["bairro"];//'Jardim Pinheiros I';
$cMun     = $_SESSION["emitente"]["codigo_cidade"];//'5200258';
$xMun     = $_SESSION["emitente"]["nome_cidade"];//'Águas Lindas de Goiás';
$UF       = $_SESSION["emitente"]["nome_estado"];//'GO';
$CEP      = preg_replace("#[^0-9]#", "",$_SESSION["emitente"]["cep"]);//'72910000';
$cPais    = '1058';
$xPais    = 'Brasil';
$fone     = preg_replace("#[^0-9]#", "", $_SESSION["emitente"]["telefone"]);//'6239324097';
$resp     = $nfe->tagenderEmit($xLgr, $nro, $xCpl, $xBairro, $cMun, $xMun, $UF, $CEP, $cPais, $xPais, $fone);
        
//Busco os Dados do Destinatario
$idDestinatario = $dadosNota["id_cliente"];
$destinatarioSelecionado = mysqli_query($conexao, "select * from clientes where id = '$idDestinatario' and id_emitente = '$id_emitente'");
$dadosDestinatario = mysqli_fetch_array($destinatarioSelecionado);
//destinatário
//$CNPJ = '23401454000170';
//$CPF = '';
if ($dadosDestinatario["pessoa"]=='J'){
  $CNPJ = preg_replace("#[^0-9]#", "",$dadosDestinatario["cpf_cnpj"]); //Numero do CNPJ Sem a Mascara
  $CPF = '';
}else{
  $CNPJ = ''; //Numero do CNPJ Sem a Mascara
  $CPF = preg_replace("#[^0-9]#", "",$dadosDestinatario["cpf_cnpj"]); //Numero do CPF Sem a Mascara	
}
$idEstrangeiro = '';
$xNome         = $dadosDestinatario["nome"];//'Chinnon Santos - Tecnologia e Assessoria em Softwares';
$indIEDest     = '9';
//1 - Contribuinte ICMS (informar a IE do destinatário);
//2 - Contribuinte isento de Inscrição no cadastro de Contribuintes do ICMS;
//9 - Não Contribuinte, que pode ou não possuir Inscrição Estadual no Cadastro de Contribuintes do ICMS.
$IE            = $dadosDestinatario["ie"];
$ISUF          = '';
$IM            = '';//'4128095';
$email         = $dadosDestinatario["email"];//'nfe@chinnonsantos.com';
$resp = $nfe->tagdest($CNPJ, $CPF, $idEstrangeiro, $xNome, $indIEDest, $IE, $ISUF, $IM, $email);

//Endereço do destinatário
$xLgr         = $dadosDestinatario["endereco"];//'Av. Vila Alpes';
$nro          = $dadosDestinatario["numero"];//'s/n';
$xCpl         = $dadosDestinatario["complemento"];//'';
$xBairro      = $dadosDestinatario["bairro"];//'Vila Alpes';
$cMun         = $dadosDestinatario["codigo_cidade"];//'5208707';
$xMun         = $dadosDestinatario["nome_cidade"];//'Goiânia';
$UF           = $dadosDestinatario["nome_estado"];//'GO';
$CEP          = preg_replace("#[^0-9]#", "",$dadosDestinatario["cep"]);//'74310010';
$cPais        = '1058';
$xPais        = 'Brasil';
$fone         = preg_replace("#[^0-9]#", "",$dadosDestinatario["telefone"]);//'6292779404';
...
//Busco os Itens da Nota
$itensNota      = mysqli_query($conexao, "select * from nota2 where id_nota = '$id_nota' and id_emitente = '$id_emitente'");
//produtos 1 (Limite da API é de 56 itens por Nota)
$numeroItem = 1;
while ($dadosItensNota = mysqli_fetch_array($itensNota)) {
    $nItem    = $numeroItem;
    $cProd    = $dadosItensNota["id_produto"];//$prod['cProd'];
    $cEAN     = '';//$dadosItensNota["codigo_ean"];
    $xProd    = $dadosItensNota["nome"];
    $NCM      = $dadosItensNota["ncm"];
    $NVE      = '';
    $CEST     = $dadosItensNota["cest"];
    $EXTIPI   = '';
    $CFOP     = $dadosItensNota["cfop"];
    $uCom     = $dadosItensNota["un"];
    $qCom     = number_format($dadosItensNota["qtd"], 4, '.', '');//$prod['qCom'];
    $vUnCom   = $dadosItensNota["valor_un"];//$prod['vUnCom'];
    $vProd    = $dadosItensNota["valor_total"];//$prod['vProd'];
    $cEANTrib = '';
    $uTrib    = $dadosItensNota["un"];//$prod['uTrib'];
    $qTrib    = number_format($dadosItensNota["qtd"], 4, '.', '');//$prod['qTrib'];
    $vUnTrib  = $dadosItensNota["valor_un"];//$prod['vUnTrib'];
    $vFrete   = '';//$dadosItensNota["frete"];//$prod['vFrete'];
    $vSeg     = '';//$prod['vSeg'];
    $vDesc    = '';//$prod['vDesc'];
    $vOutro   = '';//$prod['vOutro'];
    $indTot   = '1';//$prod['indTot'];
    $xPed     = $dadosNota["id"];//$prod['xPed'];
    $nItemPed = $numeroItem;//$prod['nItemPed'];
    $nFCI     = '';//$prod['nFCI'];	
...
$nIte       = $numeroItem; //produtos 1
$orig       = '0';
$cst        = '400';//'00'; // Tributado Integralmente
$modBC      = '3';
$pRedBC     = '';
$vBC        = $dadosItensNota["valor_total"];//'12.00';//'840.00'; // = $qTrib * $vUnTrib
$pICMS      = '0.00';//'27.00'; // Alíquota do Estado de GO p/ 'NCM 2203.00.00 - Cervejas de Malte, inclusive Chope'
$vICMS      = '0.00';//'226.80'; // = $vBC * ( $pICMS / 100 )
$vICMSDeson = '';
$motDesICMS = '';
$modBCST    = '';
$pMVAST     = '';
...
$vICMSSTRet = '';
//Se for SImples
$resp = $nfe->tagICMSSN($nItem, $orig, $cst, $modBC, $pRedBC, $vBC, $pICMS, $vICMS, $vICMSDeson, $motDesICMS, $modBCST, $pMVAST, $pRedBCST, $vBCST, $pICMSST, $vICMSST, $pDif, $vICMSDif, $vICMSOp, $vBCSTRet, $vICMSSTRet);
//Regime normal
...
$nItem = $numeroItem; //1; //produtos 1
$cst      = '53';//'50'; // 50 - Saída Tributada (Código da Situação Tributária)
$clEnq = '';
$cnpjProd = '';
$cSelo = '';
$qSelo = '';
$cEnq  = '999';
$vBC   = '0';
$pIPI = '0';//'6.00'; //Calculo por alíquota - 6% Alíquota GO.
$qUnid = '';
$vUnid = '';
$vIPI = '0';//'50.40'; // = $vBC * ( $pIPI / 100 )
...
$nItem     = $numeroItem; //1; //produtos 1
$cst       = '07';//'03'; //Operação Tributável (base de cálculo = quantidade vendida x alíquota por unidade de produto)
$vBC       = ''; 
$pPIS      = '';
$vPIS      = '0';
$qBCProd   = '0';
...
$nItem = $numeroItem; //produtos 1
$cst = '03'; //Operação Tributável (base de cálculo = quantidade vendida x alíquota por unidade de produto)
$vBC = '';
$pCOFINS = '';
$vCOFINS = '0.00';
$qBCProd = '0.0000';   //4 Casas decimais
$vAliqProd = '0.0000';  //4 Casas decimais
$resp = $nfe->tagCOFINS($nItem, $cst, $vBC, $pCOFINS, $vCOFINS, $qBCProd, $vAliqProd);
	
	
$numeroItem = $numeroItem +1; //Incremento na Variavel Item
}
//Fim da implementação dos Itens na NFe
...
$vBCST       = '0.00';
$vST         = '0.00';
$vProd       = $dadosNota["valor_produtos"]; //'12.00';
$vFrete      = $dadosNota["valor_frete"];
$vSeg        = '0.00';
$vDesc       = $dadosNota["desconto"];
$vII         = '0.00';
$vIPI        = '0.00';
$vPIS        = '0.00';
$vCOFINS     = '0.00';
...
//frete
$modFrete = $dadosNota["frete"]; //0=Por conta do emitente; 1=Por conta do destinatário/remetente; 2=Por conta de terceiros; 9=Sem Frete;
...
$infAdFisco = $dadosNota["inf_ad_fisco"];
$infCpl = $dadosNota["inf_ad_contribuinte"] ." - " . $textoIBPT;//"Pedido Nº16 - {$textoIBPT} ";
...
if ($resp) {
   // header('Content-type: text/xml; charset=UTF-8');
    ...
    chmod($filename, 0777);
	//Gravo a Chave de Acesso no Banco de Dados
	$gravaChave = mysqli_query($conexao,"update nota1 set chave_nfe = '$chave' where id = '$numeroNota'");
